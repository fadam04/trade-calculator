...
#ax1
     ...
     #stop_loss = float(stop_lost_custom_entry.get())
     #take_profit = float(tp_custom_entry.get())
     
     # Draw horizontal lines for stop loss and take profit
     #ax1.axhline(y=stop_loss, color='red', linestyle='--', label='Stop Loss')
     #ax1.axhline(y=take_profit, color='green', linestyle='--', label='Take Profit')
...
"""# Első sor (cím középen, oszlopokat egyesítve)
title_frame = ttk.LabelFrame(root,text="Trade calculator",  padding=10)
title_frame.grid(row=0, column=0, columnspan=4, padx=5, pady=5, sticky="nsew")
ttk.Label(title_frame, text="Trade Calculator", font=("Arial", 16, "bold")).grid(row=0,column=1,columnspan=3, padx=5,pady=5,sticky="ew")

# Oszloparányok beállítása
title_frame.columnconfigure(0, weight=0)  # Gear icon oszlopa fix
title_frame.columnconfigure(1, weight=1)  # Cím oszlopa rugalmas
title_frame.columnconfigure(2, weight=1)  # Középre igazításhoz segít
title_frame.columnconfigure(3, weight=1)  # Maradék helyet is kitöl
"""
...
button = tk.Button(market_frame, text="Kiválasztás", command=show_selection)
...
tk.Label(trade_frame, text="Curency:").grid(row=2, column=0, padx=10, pady=5, sticky="N")
